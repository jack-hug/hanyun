...
class AddProductForm(FlaskForm):
    name = StringField('Name', validators=[DataRequired(), Length(1, 100)])
    category = SelectField('Category', coerce=int)
    price = StringField('Price', validators=[DataRequired(), Length(1, 20)], default='0.0')
    material = StringField('Material', validators=[Length(0, 200)], default='silicone')
    level = StringField('Level', validators=[Length(0, 200)], default='High Quality')
    oem = StringField('OEM/ODM', validators=[Length(0, 200)], default='Welcome')
    content = CKEditorField('Content')
    photos = FileField('Product Photo:', validators=[FileAllowed(['jpg', 'png', 'gif'], '只能上传图片')])
    submit = SubmitField('Submit')
...
def contact():
    ...
    if messageform.validate_on_submit():
        ...
        # send_new_message_email(messageform.email.data, messageform.content.data)  暂停发邮件功能
        messageform.name.data = ''
        messageform.email.data = ''
        messageform.content.data = ''
